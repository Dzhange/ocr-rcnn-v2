...
def detector():
    ...
    if request.method == 'POST':
        ...
        boxes, scores, _ = detector.predict(img, True)
        button_patches, button_positions, _ = button_candidates(boxes, scores, img)
        ...
        for button_img, button_pos in zip(button_patches, button_positions):
          ...
          location_list.append(button_pos)
          text_list.append(button_text)        
        ...
        # print(button_text)
        #cv2.imwrite("./result_received.jpg",detector.image_show)
        
        #if len(text_list) == 0:
        #  out = {"location":'', "text":''}
        #else:
        out = {"location":json.dumps(location_list), "text":json.dumps(text_list)}
        
        print("[ INFO ] done {}".format(str(text_list))) 
    return out
...
if __name__ == '__main__':
    ...
    # img_path = './test_panels/image0.jpg'
